...
SizeFems1lm <- brm(N15~1+(1|BearID)+(1|fYR),
                   data = FemsOnly,
                   family = gaussian(link = "log"))
options(mc.cores = 1)
fit7 <- add_criterion(SizeFems1lm, criterion = "loo")
gc()
print(fit7$criteria$loo)
FemsINT.LOOic <- fit7$criteria$loo$estimates[3,1]

SizeFems2lm <- brm(N15~headcirc+(1|BearID)+(1|fYR),
                   data = FemsOnly,
                   family = gaussian(link = "log"))
fit8 <- add_criterion(SizeFems2lm, criterion = "loo")
gc()
print(fit8$criteria$loo)
FemsHC.LOOic <- fit8$criteria$loo$estimates[3,1]
...
DietEst.Fems<- read.csv("DietEstFeamles.csv")
...
DietEst.Males <- read.csv("DietEstMales.csv")
...
MooseMed.F <- median(DietEst.Fems$Moose)*100
MooseSD.F <- sd(DietEst.Fems$Moose)*100

MooseMed.M <- median(DietEst.Males$Moose)*100
MooseSD.M <- sd(DietEst.Males$Moose)*100
...
IndMales <- subset(Mixture.RAW,sex=="M")
IndMales <- subset(IndMales, reprostatus.represent=="SolitaryM")
IMid <- as.factor(IndMales$BearID)
as.numeric(IMid)

IndFemales <- subset(Mixture.RAW,sex=="F")
IndFemales <- subset(IndFemales,
                     reprostatus.represent=="SolitaryF")
IFid <- as.factor(IndFemales$BearID)
as.numeric(IFid)
